...
- **`/test/:data` (`fakerfq` ág) — kódba égetett, éles adatbázisból származó `customer_id` lista**: a `generateRandomRFQ()` teszt-/demóadat-generátor egy fix, `[2, 5, 6, 7, ..., 16]` tartalmú tömbből választott véletlenszerű `customer_id`-t az RFQ-rekordokhoz — ez nyilvánvalóan az akkori éles (AWS RDS) adatbázis konkrét ügyfél-azonosítóinak pillanatfelvétele volt. A Report oldal modernizálásának valós böngészős tesztelése (lásd `kod_atvilagitas_kliens.md`, 7.2. pont) közben derült ki, hogy ez bármely friss/üres adatbázison (pl. `/setup` telepítő varázslóval frissen létrehozott admin fiók + néhány teszt-ügyfél esetén) idegen kulcs kényszer megsértésével (`rfq_customer_id_fkey`) azonnal elbukik, mivel a hivatkozott azonosítók nem léteznek. Javítva: a `/test/:data` végpont a `fakerfq` ág elején lekérdezi a `customers` tábla tényleges `customer_id` értékeit, és ezt adja át a generátornak; üres `customers` tábla esetén egyértelmű hibaüzenettel (400) tér vissza ahelyett, hogy a ciklus közepén, kevéssé informatív adatbázis-hibával bukna el.
- **`updateProfile()` — a Profile oldal "Update" gombja a cégadat-mezők 5/9-ét csendben soha nem mentette el**: a függvény kizárólag az `addresses` táblát frissítette/szúrta be (ország, város, irányítószám, cím), miközben a kliens (`profile.ejs`, `profile_update()`) az `id` és a négy cím-mező mellett a `company`/`vat`/`c_name`/`email`/`phone` mezőket is elküldte — ezeket a szerver korábban egyszerűen figyelmen kívül hagyta. A felhasználó ennek ellenére minden alkalommal "Profile updated successfully" visszajelzést kapott, hiszen az `addresses`-műveletek sikeresen lefutottak — a cégnév/adószám/kapcsolattartó/e-mail/telefon mezők módosítása tehát tartósan, hibaüzenet nélkül elveszett. A hiba a Profile oldal modernizálásának végpontok közötti (adatbázis-szintű) ellenőrzése során derült ki: a szerkesztett `contact_name` értéke közvetlen SQL-lekérdezéssel ellenőrizve nem változott, holott a felület sikert jelzett. Javítva: `updateProfile(data)` mostantól előbb egy `UPDATE customers SET customer_name = ..., vat_number = ..., contact_name = ..., email = ..., phone = ... WHERE customer_id = ...` lekérdezést futtat le, majd — változatlanul — az `addresses` UPDATE-vagy-INSERT logikát; emellett a `customers.email` oszlopon lévő egyedi index (`004`-es migráció) `23505` (unique_violation) hibakódját is kezeli, ilyenkor 409-es válasszal és felhasználóbarát magyar hibaüzenettel ("Ez az e-mail cím már foglalt.") tér vissza ütköző e-mail-cím esetén, adatbázis-hiba helyett.
