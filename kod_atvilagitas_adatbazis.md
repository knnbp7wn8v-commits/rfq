...
- **`rfq.certification` adatminőségi kérdés (3. pont)**: **teljes egészében megvalósult**, mindkét ütemezett lépéssel. Rövid táv: `db/migrations/007_certification_check.sql` (meglévő "true"/"false" értékek normalizálása "yes"/"no"-ra + `CHECK` kényszer), valamint a `generateRandomRFQ()` teszt-generátor (`app.js`) javítása. Középtáv: `db/migrations/008_certification_boolean.sql` — a `cart.certification`/`rfq.certification` oszlopok tényleges `boolean` típusra alakítása (`USING (certification = 'yes')`, kihasználva, hogy a PostgreSQL natívan `boolean`-ként értelmezi a `'yes'`/`'no'` szöveges literálokat, ezért a beszúrási útvonalakon — `addToCart`, `insertRFQ`, `fn_create_order_from_cart` — nem volt szükség kódmódosításra), a redundánssá vált `CHECK` kényszerek eltávolítása, valamint a `fn_report_certification_demand` függvény visszatérési típusának igazítása (`DROP FUNCTION` + újradefiniálás, mivel a `CREATE OR REPLACE FUNCTION` nem képes visszatérési típust módosítani). A megjelenítési rétegben (`views/report.ejs`, `views/admin.ejs`, `public/assets/scripts/cart.js`) a logikai érték "yes"/"no" szöveggé alakítása történt vissza a felhasználó felé.
...
- **Kiegészítő javítás, a 2. lépés (FK-k) bevezetése által feltárt hiba**: a `customers_role_id_fkey` (`005_foreign_keys.sql`) egy teljesen friss, üres adatbázison azonnal láthatóvá tette, hogy a `permissionsets`/`roles` táblák tartalma sosem volt automatizálva — csak a manuálisan, korábban feltöltött éles (AWS RDS) adatbázison léteztek. Egy friss adatbázison ez a `/setup` (első admin létrehozása) végpont `23503` (foreign key violation) hibával történő elbukását okozta volna, mivel az alkalmazás az `role_id = 3` értéket kódba égetve várja el (`ROLES.ADMIN`). Javítva: `db/migrations/009_seed_roles.sql`, amely `ON CONFLICT DO NOTHING` móddal — tehát a már feltöltött éles adatbázison ténylegesen hatástalanul — felveszi a rögzített referenciaadatot. A javítást egy teljes, friss adatbázison végigfuttatott E2E teszt (migrációk → `/setup` → bejelentkezés → kosár → megrendelés → riport → admin nézet) igazolta.
- **Kiegészítő javítás, a felhasználó jelzése alapján — termékportfólió referenciaadatok**: a `diameter`, `ediam_param`, `eheight`, `grammatura`, `permissionsets`, `plie_param`, `plies`, `portfolio`, `reels`, `tds`, `tissue`, `truck`, `weight` táblák tartalma — a gyártó termékportfólióját leíró, gyakorlatilag rögzített törzsadat — eddig kizárólag kézileg, a hiányos adminisztrációs felület megkerülésével került fel a már meglévő éles adatbázisra; a migrációk ezt eddig nem tartalmazták, így egy teljesen friss adatbázison az ajánlatkérési folyamat legördülő listái üres állapotból indultak volna. Javítva: `db/migrations/010_seed_reference_data.sql`, az éles export (`mydb_backup.sql`) e táblákra vonatkozó tényleges tartalmából generálva, `ON CONFLICT DO NOTHING` móddal (az éles adatbázison hatástalan). A `customers`/`addresses`/`cart`/`rfq` táblák — mivel személyes adatot (valós ügyfél-email/telefon, jelszó-hash) tartalmaznak — szándékosan kimaradtak ebből a migrációból. Ellenőrizve: friss adatbázison a migráció hibamentesen lefut, a sorszámok (`diameter`: 4, `tds`: 108, `portfolio`: 336 stb.) megegyeznek az éles adattal, és a migráció kétszeri lefuttatása mellett sem duplázódnak a sorok (idempotencia-teszt).
